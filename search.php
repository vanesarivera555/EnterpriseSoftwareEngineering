...
if (!isset($_POST['submit']))
{
    ...
    echo '<option value="LoanNumber">Loan Number</option>';
	echo '<option value="all">All</option>';
	echo '</select>';
	echo '<hr>';
	echo '<button type="submit" class="btn btn-success" name="submit" value="submit">Search</button>';
	echo '</form>';
}
if (isset($_POST['submit']))
{
	$searchType=$_POST['searchType'];
	$searchString=addslashes($_POST['searchString']); //addslashes to escape any special chars instead of getting parsed it gets encapsulated
	switch($searchType)
	{
		case "name":
		...
		case "uploadDate":
			...
			break;
        case "category":
            $sql="Select `name`,`upload_date`,`upload_by`,`auto_id` from `documents` where `category` like '%$searchString%'";
            break;
        case "LoanNumber":
        $sql="Select `name`,`upload_date`,`upload_by`,`auto_id` from `documents` where `loan_number` like '%$searchString%'";
            break;
		case "all":
			$sql="Select `name`,`upload_date`,`upload_by`,`auto_id` from `documents`";
			break;
		default:
			redirect("search.php?msg=searchTypeError");   //Good spot to log error
			break;
	}
